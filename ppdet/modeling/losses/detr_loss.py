...
__all__ = ['DETRLoss', 'DINOLoss','DINOLoss_semi','DETRLoss_semi']
@register
class DETRLoss_semi(nn.Layer):
    ...
    def __init__(self,
        ...
        """
        super(DETRLoss_semi, self).__init__()
    ...
    def _get_loss_bbox(self, boxes, gt_bbox,gt_ious, match_indices, num_gts,
                       postfix="",iou_scores=None):
    ...
    def _get_loss_aux(self,
        ...
        if boxes is None or logits is None:
            return {
                ...
                "loss_iou_aux" + postfix: paddle.paddle.zeros([1])
            }
        loss_class = []
        loss_bbox = []
        loss_giou = []
        ...
        if dn_match_indices is not None:
            ...
            match_indices = self.matcher(boxes[self.same_match_ind],
                                         ...
                                         gt_class)
        for aux_boxes, aux_logits,aux_ious in zip(boxes, logits,iou_scores):
            ...
            if self.use_vfl:
                ...
                iou_score = None
            loss_class.append(
                self._get_loss_class(aux_logits, gt_class,gt_iou, match_indices,
                                     bg_index, num_gts, postfix, iou_score)[
                                         'loss_class' + postfix])
            loss_ = self._get_loss_bbox(aux_boxes, gt_bbox,gt_iou, match_indices,
                                        num_gts, postfix,iou_scores=aux_ious)
            loss_bbox.append(loss_['loss_bbox' + postfix])
            loss_giou.append(loss_['loss_giou' + postfix])
            loss_iou.append(loss_['loss_iou' + postfix])
        loss = {
            ...
            "loss_iou_aux" + postfix: paddle.add_n(loss_iou)
        }
        return loss
    ...
    def forward(self,
        ...
        total_loss.update(
            self._get_loss_class(logits[
                -1] if logits is not None else None, gt_class,gt_ious,match_indices,
                                 self.num_classes, num_gts, postfix, iou_score))
        total_loss.update(
            self._get_loss_bbox(boxes[-1] if boxes is not None else None,
                                gt_bbox,gt_ious, match_indices, num_gts, postfix,iou_scores=ious[-1] if ious is not None else None))
        ...
        if self.aux_loss:
            total_loss.update(
                self._get_loss_aux(
                    ...
                    if logits is not None else None,gt_bbox, gt_class,gt_ious,
...
class DETRLoss(nn.Layer):
    ...
    def _get_loss_bbox(self, boxes, gt_bbox, match_indices, num_gts,
        ...
        src_bbox, target_bbox = self._get_src_target_assign(boxes, gt_bbox,
                                                            match_indices)
        src_score, _ = self._get_src_target_assign(iou_scores, gt_bbox,
                                                            match_indices)   
...
class DINOLoss(DETRLoss):
    def forward(self,
                ...
                dn_meta=None,
                **kwargs):
        total_loss = super(DINOLoss, self).forward(boxes, logits,ious, gt_bbox,
                                                   gt_class,)
...
@register
class DINOLoss_semi(DETRLoss_semi):
    def forward(self,
                boxes,
                logits,
                ious,
                ...
                dn_meta=None,
                **kwargs):
        total_loss = super(DINOLoss_semi, self).forward(boxes, logits,ious, gt_bbox,
                                                   gt_class,gt_iou)
        ...
        # compute denoising training loss
        dn_loss = super(DINOLoss_semi, self).forward(
            dn_out_bboxes,
            dn_out_logits,
            dn_out_ious,
            gt_bbox,
