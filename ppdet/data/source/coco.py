...
class COCODataSet(DetDataset):
    ...
    def __init__(self,
        ...
        if proposal_file is not None:
            if not isinstance(proposal_file, Sequence):
                proposal_file = [proposal_file]
            self.imgid2proposal = [
                self._load_proposal(f) for f in proposal_file
            ]
        else:
    ...
    def _get_proposal(self, img_id, gt_num):
        ...
        for imgid2proposal in self.imgid2proposal:
            proposal_bbox, proposal_class, proposal_score = [], [], []
            for proposal in imgid2proposal[img_id]:
            ...
            proposal_bbox = np.array(
                proposal_bbox,
                dtype=np.float32) if len(proposal_bbox) > 0 else np.zeros(
                    (0, 4), dtype=np.float32)
            proposal_class = np.array(
                proposal_class,
                dtype=np.int32) if len(proposal_class) > 0 else np.zeros(
                    (0, 1), dtype=np.int32)
            proposal_score = np.array(
                proposal_score,
                dtype=np.float32) if len(proposal_score) > 0 else np.zeros(
                    (0, 1), dtype=np.float32)

            proposal_num = min(self.proposal_num, 2 * gt_num)
            if len(proposal_bbox) > proposal_num:
            ...
            proposal_classes.append(proposal_class)
            proposal_scores.append(proposal_score)

        return proposal_bboxes, proposal_classes, proposal_scores
        ...
        for img_id in img_ids:
            ...
            if not self.load_image_only:
                ...
                if self.imgid2proposal is not None:
                    proposal_bboxes, proposal_classes, proposal_scores = self._get_proposal(
                        img_id, len(gt_bbox))
                    gt_rec.update({
                        'proposal_bbox': proposal_bboxes,
                        'proposal_class': proposal_classes,
